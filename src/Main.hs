...
main = do
  ...
  let getConnection h p | tls config = tlsConnection (WithDefaultConfig h p)
                        | otherwise = plainConnection h p
      authenticate | Just pass <- password config =
                      send $ Privmsg "NickServ" $ Right $ T.unwords ["IDENTIFY", nick config, pass]
                   | otherwise = pure ()
