...
- `URL` fetches and posts URL titles.
- `Eval` provides evaluators for a bunch of languages using [qeval](https://github.com/ncfavier/qeval). Syntax: `LANG> EXPRESSION`. Use `↵` (compose + return) to insert newlines.
- `Wikimedia` queries information from Wikimedia projects.
...
- `NLab` queries information from the [nLab](https://ncatlab.org), a wiki for higher mathematics.
  - `.nlab SEARCH` searches for a page and prints a link to the first result.
  - `.nlabrandom [CATEGORY]` prints a random page.
- `MerriamWebster` queries information from the [Merriam-Webster](https://www.merriam-webster.com) dictionary.
  - `.d[ef[ine]] [-e ENTRY] WORD` queries a word's definition.
  - `.etym[ology] [-e ENTRY] WORD` queries a word's etymology.
- `Translate` provides translations via [Google Translate](https://translate.google.com/).
...
- `Typst` renders [Typst](https://typst.app/) markup to HTML and uploads the result (see `config.pasteUrl`). Syntax: `typst> MARKUP`.
- `WolframAlpha` queries [WolframAlpha](https://www.wolframalpha.com) for answers.
